...
export const getProductType = async (type,page,limit) => {
 if (type) {
  const res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-all?page=${page}&filter=type&filter=${type}&limit=${limit}`);
  return res.data;
 }
...
export const getDetailsProduct = async (id,data) => {
  const res = await axios.get(`${process.env.REACT_APP_API_URL}/product/get-details/${id}`,data);
  return res.data;
};
